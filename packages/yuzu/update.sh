...
updateEarlyAccess() {
    ...
    NEW_EA_VERSION="$(curl -sL "https://api.github.com/repos/pineappleEA/pineapple-src/releases" | jq 'map(select(.tag_name != "continuous")) | .[0].tag_name' --raw-output | cut -d"-" -f2 | cut -d" " -f1)"
