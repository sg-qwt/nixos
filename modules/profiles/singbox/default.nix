...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    security.acme.certs.edg = mkIf (cfg.profile == "anytls") {
      ...
      postRun = "${lib.getExe' pkgs.acl "setfacl"} --recursive --modify u:sing-box:rX ${edgCertDir}";
    };

    security.acme.certs.cybcc = mkIf (cfg.profile == "hysteria") {
      reloadServices = [ "sing-box.service" ];
      postRun = "${lib.getExe' pkgs.acl "setfacl"} --recursive --modify u:sing-box:rX ${cybccCertDir}";
    };
